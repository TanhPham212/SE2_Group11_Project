				...
				$.each(result.content, function(i, donHang){
					...
					var donHangRow = '<tr>' +
					     ...
					     if(donHang.trangThaiDonHang == "Đang chờ giao" || donHang.trangThaiDonHang == "Đang giao"){
					    	 donHangRow += ' &nbsp;<button class="btn btn-primary btnPhanCong">Assign</button>'+
...
//		console.log(donHangId);
		...
		$.get(href, function(donHang) {
			$('#maDonHang').text("Id: "+ donHang.id);
			$('#hoTenNguoiNhan').text("Receiver: "+ donHang.hoTenNguoiNhan);
			$('#sdtNhanHang').text("Phone: "+ donHang.sdtNhanHang);
			$('#diaChiNhan').text("Address: "+ donHang.diaChiNhan);
			$('#trangThaiDonHang').text("Status: "+ donHang.trangThaiDonHang);
			$("#ngayDatHang").text("Date: "+ donHang.ngayDatHang);
			
			if(donHang.ngayGiaoHang != null){
				$("#ngayShipHang").text("Deli date: "+ donHang.ngayGiaoHang);
			}
			
			if(donHang.ngayNhanHang != null){
				$("#ngayNhanHang").text("Rec date: "+ donHang.ngayNhanHang);
			}
			...
			if(donHang.nguoiDat != null){
				$("#nguoiDat").html("<strong>Customer</strong>:  "+ donHang.nguoiDat.hoTen);
			}
			...
			var check = donHang.trangThaiDonHang == "Hoàn thành" || donHang.trangThaiDonHang == "Chờ duyệt" ;
			if(check){
				$('.chiTietTable').find('thead tr').append('<th id="soLuongNhanTag" class="border-0 text-uppercase small font-weight-bold"> Quantity </th>');
			}
			// thêm bảng:
