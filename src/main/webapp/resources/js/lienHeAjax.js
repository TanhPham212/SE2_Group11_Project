				...
				$.each(result.content, function(i, lienHe){
					var lienHeRow = '<tr>' +
					        ...
					        lienHeRow += '<td width="0%">'+'<input type="hidden" id="lienHeId" value=' + lienHe.id + '>'+ '</td>'+
                                      ...
                                      '&nbsp; <button class="btn btn-warning btnTraLoi" >Response</button></td>';
...
//		event.preventDefault();
	...
	$(document).on('click', '.btnChiTiet', function (event){
		...
		$.get(href, function(lienHe) {
			$('#noiDungLienHe').html("<strong>Content</strong>: <br> "+ lienHe.noiDungLienHe);
			$('#ngayLienHe').html("<strong>Date</strong>: "+ lienHe.ngayLienHe);
			$('#email').html("<strong>Email </strong>: "+ lienHe.emailLienHe);
			$('#trangThai').html("<strong>Status</strong>: "+ lienHe.trangThai);
			
			if(lienHe.trangThai == "Đã trả lời"){
			  $('#noiDungTraLoi').html("<strong>Content</strong>: "+ lienHe.noiDungTraLoi);
			  $('#ngayTraLoi').html("<strong>Date</strong>: "+ lienHe.ngayTraLoi);
			} else {
			    $('#noiDungTraLoi').html("");
			    $('#ngayTraLoi').html("");	
			}
